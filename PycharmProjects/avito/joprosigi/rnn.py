...
#TODO
#Relace transforamtion of test data by call to preprocess_dataset?
...
def keras_fit(train):
    ...
    test = pd.read_csv("../data/test.csv", parse_dates=["activation_date"], usecols=use_cols[:-1], dtype=dtypes_train)
...
def RNN_model():
    ...
    emb_image_code = Embedding(max_image_code, 10)(image_code)
    ...
    # model
    model = Model([seq_title_description, region, city, category_name, parent_category_name,
                   param_1, param123, price, item_seq_number, image_code,
...
def predictions(model):
    ...
    def load_test():
        for df in pd.read_csv('../data/test.csv', chunksize=250000):
            yield df
